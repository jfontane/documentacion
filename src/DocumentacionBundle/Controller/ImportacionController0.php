...
class ImportacionController extends Controller {
    ...
    public function listarAction() {
        ...
        //dump($usuarios);die;
        return $this->render('@JubilacionesDeclaraciones/Importacion/listar.html.twig', array(
                    'archivos' => $archivos
        ));
    }
    ...
    public function nuevoAction(Request $request) {
        ...
        return $this->render('@Documentacion/AdminImportacion/nuevo.html.twig', array('form' => $form->createView()
        ));
    }
...
//  $linea=explode(';',$archivo[0]);
        ...
        for ($i = 0; $i < $lineas; $i++) {
            ...
            $rep = $em->getRepository('DocumentacionBundle:Representante')->findOneBy(array('cuil' => $rep_cuil));
            if (null == $rep) {
    ...
    private function vincularOrganismosRepresentantes($fileName) {
        ...
        for ($i = 0; $i < $lineas; $i++) {
            ...
            $organismo = $em->getRepository('DocumentacionBundle:Organismo')->findOneBy(array('codigo' => $codigo_organismo));
            $representante = $em->getRepository('DocumentacionBundle:Representante')->findOneBy(array('cuil' => $cuil));
    ...
    private function vincularUsuariosOrganismos($fileName) {
        ...
        for ($i = 0; $i < $lineas; $i++) {
            $codigo_organismo = explode(';', $archivo[$i])[0];
            $organismo = $em->getRepository('DocumentacionBundle:Organismo')->findOneBy(array('codigo' => $codigo_organismo));
            $usuario = $em->getRepository('DocumentacionBundle:User')->findOneBy(array('username' => $codigo_organismo));
    ...
    private function vincularDeclaracionesOrganismos($fileName) {
        ...
        for ($i = 0; $i < $lineas; $i++) {
            ...
            if ($codigo_organismo_actual == $codigo_organismo_obtenido) {
                ...
                $organismo = $em->getRepository('DocumentacionBundle:Organismo')->findOneBy(array('codigo' => $codigo_organismo_actual));
                $declaracion->setOrganismo($organismo);
                $em->persist($declaracion);
                $em->flush();
            }
